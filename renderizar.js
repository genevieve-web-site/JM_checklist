    ...
    <section aria-labelledby="secao-${indice}">
      ...
      ${sec.itens.map(it => `<div class="item${sec.imagem ? ' design' : ''}" data-id="${it.id}">
        ...
        <div class="corpo">
          ...
          <div class="resposta"><label for="resposta-${it.id}">Sua resposta / observações</label><textarea id="resposta-${it.id}" name="${it.id}-resposta" rows="3" placeholder="Informe sua decisão, os ajustes, textos ou links necessários."></textarea></div>
        </div>
